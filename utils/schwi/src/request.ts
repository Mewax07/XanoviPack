...
class HttpRequestBuilder {
	...
	private method: HttpRequestMethod = HttpRequestMethod.GET;
	private redirection: HttpRequestRedirection = HttpRequestRedirection.MANUAL;
	private url: URL;
	...
	public build(): HttpRequest {
		...
		if (cookies.length > 0) this.headers.set(HeaderKeys.COOKIE, cookies);

		return new HttpRequest(this.url, this.method, this.body, this.headers, this.redirection);
	}
...
export class HttpRequest {
	...
	public constructor(
		...
		public readonly headers: HeaderMap | Record<string, string>,
